...
def challenges():
    ...
    m = current_member()
    rows = conn.execute(
        "SELECT * FROM challenges WHERE is_active = 1 ORDER BY category, points"
    ).fetchall()
    solved_ids = {
    ...
    return render_template("challenges.html", by_category=by_category, member=m)
...
def admin_challenges():
    ...
    conn.close()
    return render_template("admin_challenges.html", rows=rows)
